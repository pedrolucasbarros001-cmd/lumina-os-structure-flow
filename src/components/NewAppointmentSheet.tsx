        ...
        {step === 'service' && (
          <div className="flex flex-col h-full">
            <div className="flex items-center gap-3 px-4 pt-5 pb-3">
              <button onClick={() => setStep('client')} className="w-8 h-8 rounded-full hover:bg-muted flex items-center justify-center">
                <X className="w-5 h-5" />
              </button>
            ...
            <div className="flex-1 overflow-y-auto">
              {activeServices.map(s => (
                <button
                  key={s.id}
                  onClick={() => { setSelectedService({ id: s.id, name: s.name, duration: s.duration, price: s.price }); setStep('detail'); }}
                  className="w-full flex items-center justify-between px-4 py-3.5 hover:bg-muted/50 transition-colors border-b border-border/30"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-1 h-10 rounded-full bg-sky-400" />
                    <div className="text-left">
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              <div className="px-4 py-3 space-y-2.5 border-b border-border/30">
                ...
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <span>{format(parsedDatetime, 'HH:mm')} - {selectedService ? `${selectedService.duration}min` : '60min'}</span>
                </div>
                  ...
                  <div className="flex items-center gap-3 py-2">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {format(parsedDatetime, 'HH:mm')} · {selectedService.duration}min
                        {teamMember ? ` · ${teamMember.name}` : ''}
                      </p>
                    </div>
                    <span className="text-sm font-semibold">€{selectedService.price.toFixed(2)}</span>
                  </div>
                )}
            ...
            <div className="border-t border-border/50 px-4 py-4 bg-background space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-lg font-bold">€{(selectedService?.price || 0).toFixed(2)}</span>
              </div>
              <Button
