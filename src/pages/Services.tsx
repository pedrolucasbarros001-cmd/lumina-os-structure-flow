...
function AddServiceSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onClose();
      setForm({ name: '', price: '', duration: '60', description: '', is_active: true });
    } catch {
    ...
    <Sheet open={open} onOpenChange={o => !o && onClose()}>
      <SheetContent side="bottom" className="rounded-t-3xl h-[80vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Button type="submit" className="w-full" disabled={createService.isPending}>Criar Serviço</Button>
        </form>
      </SheetContent>
    </Sheet>
  );
...
function ServiceCard({ service }: { service: Service }) {
  ...
  return (
    <div className={cn(
      ...
      service.is_active ? 'bg-card border-border/50 hover:border-primary/30' : 'bg-muted/20 border-border/30 opacity-60'
    )}>
