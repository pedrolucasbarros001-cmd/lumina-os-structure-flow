...
function AppointmentBlock({
  ...
  return (
    <div
      className={cn(
        'absolute rounded-lg px-2 py-1.5 cursor-pointer overflow-hidden transition-all hover:-translate-y-0.5 shadow-sm select-none',
        isDragSource ? 'opacity-30' : ''
      )}
      style={{ top: `${top}px`, height: `${Math.max(height, 28)}px`, left, width: `calc(${colWidth} - 4px)`, zIndex: 5, backgroundColor: style.bg, color: style.text }}
      onClick={onClick}
      onPointerDown={(e) => onLongPressStart(e, appt)}
    >
      <p className="text-[10px] font-semibold leading-none truncate opacity-80" style={{ color: style.text }}>
        {formatTimeRange(appt.datetime, appt.duration || 60)}
      </p>
      <p className="text-xs font-bold truncate mt-0.5" style={{ color: style.text }}>{appt.client_name || 'Cliente'}</p>
      {isHome && <MapPin className="w-3 h-3 text-orange-600 absolute top-1.5 right-1.5" />}
      {isCompleted && (
